...
spec = do
  describe "Term" do
    describe "Term.step" do
      it "Does nothing when term is in normal form."
        $ do
            "x" `shouldStepTo` "x"
            "λx.x" `shouldStepTo` "(λx.x)"
            "λx.λy.λz. x y z" `shouldStepTo` "(λx.(λy.(λz.((x y) z))))"
            "x y" `shouldStepTo` "(x y)"
      it "Substitutes argument for all occurences of function parameter."
        $ do
            "(λx.x) y" `shouldStepTo` "y"
            "(λx.x x z x) y" `shouldStepTo` "(((y y) z) y)"
      it "Does not substitute where outer parameter is shadowed by inner parameter."
        $ do
